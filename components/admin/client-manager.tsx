...
const rewardMilestones = [
  { visits: 3, label: "French free" },
  { visits: 5, label: "10% OFF" },
  { visits: 7, label: "Charms free" },
...
export function ClientManager({ mode = "clients" }: { mode?: "clients" | "appointments" }) {
  ...
  async function addVisit(client: Profile, direction: 1 | -1) { const before = Number(client.visit_count || 0); const next = Math.max(0, before + direction); await updateClient(client, { visit_count: next }); const reward = direction === 1 ? unlockedReward(before, next) : null; if (reward) await supabase.from("client_notifications").insert({ client_id: client.id, title: "Reward disponibil", message: `Ai ajuns la ${reward.visits} vizite. Ai deblocat: ${reward.label}.`, type: "reward" }); }
  async function addAppointment(e: React.FormEvent) {
    ...
    if (sendNotifications) {
      await supabase.from("client_notifications").insert([
        { client_id: selectedClient, title: "Programare confirmată", message: `Programarea ta este confirmată pentru ${formatDate(date)} la ${time}.${customNote ? ` ${customNote}` : ""}`, type: "appointment", status: "sent" },
        { client_id: selectedClient, title: "Reminder programare", message: `Reminder 48h: ai programare pe ${formatDate(date)} la ${time}.`, type: "appointment_reminder", status: "scheduled_48h" },
        { client_id: selectedClient, title: "Reminder programare", message: `Reminder 24h: mâine ai programare la ${time}.`, type: "appointment_reminder", status: "scheduled_24h" },
        { client_id: selectedClient, title: "Reminder programare", message: `Reminder 3h: ne vedem curând la programare.${customNote ? ` ${customNote}` : ""}`, type: "appointment_reminder", status: "scheduled_3h" },
      ]);
    }
    setDay(""); setTime("09:00"); setNote(""); setCustomNote(""); setMessage("Programare adăugată. Reminderele au fost pregătite în contul clientei."); await loadAll();
  }
